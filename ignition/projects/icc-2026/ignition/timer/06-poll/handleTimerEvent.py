def handleTimerEvent():
    ...
    Thin on purpose. Every decision lives in `metone_poll`: the cursor walk, the
    dedupe floor, the excursion flag, store-before-publish. The only thing that
    belongs in a timer is the cadence.
    ...
    **The cadence is `attributes.delay` in resource.json, not
    ...
    top of itself. docs/plans/06-poll-metone.md, "Open items".
