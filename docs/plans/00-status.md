...
[`00-post-07.md`](00-post-07.md) — written 2026-08-30, still the current step, with § 3 now
closed. All seven patterns are built and all seven talk tracks are written, so what is left of
...
| **03 — OPC UA → MQTT** | Analyzer path built and broker-verified 2026-08-20 (`S-00140` watched live). Device id is `cell-analyzer-01`. The Countess is out of the demo. **2026-08-26: the instrument gained its own sample-login screen on :8087 and no longer free-runs** — `SAMPLE_INTERVAL_S` defaults to 0, and the sample id is typed in by a person. **Talk track written 2026-09-06.** [`03-opcua-analyzer-playbook.md`](03-opcua-analyzer-playbook.md) § 8b, [`../talk-tracks/03-opcua-analyzer.md`](../talk-tracks/03-opcua-analyzer.md). |
| **04 — LIMS webhook** | Verified end-to-end 2026-08-20: ingest, reject, atomic approve, webhook publish, 409 replay, 401 wrong secret, and outbox survival across `docker restart icc26-lims`. **Rebuilt 2026-08-26: the LIMS opens the sample entry from pattern 1's `event/sample-acq-completed` and appends the analyzer result to it** — new `lims.sample` table, unmatched-result parking and reattach, valve provenance on the released document. **Broker-verified end to end 2026-08-26** (`S-20260826-0009` watched live): valve event opens the entry, the transcribed id brings the analyzer result onto it, approve publishes `values.collection` beside the analytes. Failure paths verified too — transposed id parks and reattaches with the wrong id preserved, a sagged air supply yields a `failed-to-seat` entry reviewable with no analytes, and a `docker restart icc26-lims` logs the retained redelivery as a no-op. ~~**Remaining:** both review outcomes publish `analyst` + `disposition`.~~ **Closed 2026-08-30** — `reject()` writes its outbox row in the same transaction as the status update and the envelope carries `disposition` `pass` on approve, `fail` on reject; a silent rejection would have left 07 with half a trigger. **Talk track rewritten 2026-09-06** for the 2026-08-26 rebuild it had never described. [`04-lims-webhook.md`](04-lims-webhook.md) § *Revised 2026-08-26*, [`../talk-tracks/04-lims-webhook.md`](../talk-tracks/04-lims-webhook.md). |
| **05 — CDC** | **Built and broker-verified 2026-08-26.** Click `manual_advance` in Tag Explorer → two rows in `bes.batch_event` → Debezium → `cdc-sink` → `icc26/site1/upstream/br-201/batch/event`, watched live with LSNs on the wire. `qualified_window` correct on both polarities. The batch engine is a **manual advance**, not the master plan's timer, and the ISA-88 element is an **operation**, not a phase. Three transport traps cost the build time — the Debezium config mount path, Ignition's HTTP→HTTPS redirect versus a Java client that does not follow redirects, and a stale container surviving `docker compose up -d`. **Treat it as an MVP** — it proves the mechanism but has not been shaped by a consumer, because 07 is not written. The `values` shape, the two-rows-per-click model and the empty `batch_end` are all open to change when 07 lands; nothing writes `deviation` yet. ~~**Remaining:** the talk track.~~ **Written 2026-09-06**, after 07 landed — and 07 consumed `bes.batch_event` exactly as the MVP shaped it, so none of the five open shapes had to change. [`05-cdc-batch-event.md`](05-cdc-batch-event.md) § *This is an MVP*, [`../talk-tracks/05-cdc.md`](../talk-tracks/05-cdc.md) |
| **06 — poll** | **Built and broker-verified 2026-08-29.** `services/sim-particle-counter` serves the vendor GraphQL API over HTTPS on 8443 with an operator touchscreen on 8089; Ignition's `particle_counter_poll` walks the cursor, writes `em.reading`, and relays through Event Stream `06_poll/particle-counter-result` to `icc26/site1/env_monitoring/particle-counter-01/sample-analyzed`. 96 analyses to 96 rows to 96 messages, `status` correct on both polarities, and the stale-cursor trap reproduced and recovered with one tag. Three predictions in the spec were wrong and are corrected there: `publishEvent` needs a JSON string, value persistence is a provider setting not a per-tag one, and the store cannot dedupe on `sequence_number`. **The gateway timer landed the same evening** — `ignition/timer/06-poll/`, 30 s fixed delay, schema learned from an empty resource created in the Gateway UI. **All ten checkpoints are closed** and pattern 6 runs hands-off: a 7.2 / 17.2 / 27.2 s lag sawtooth, three analyses per poll, and the wire shows a burst of three every 30 s rather than one message every 10 s. ~~**Remaining:** the talk track.~~ **Written 2026-09-06**, ending on the stale cursor recovered live. **Revised 2026-09-13: the topic and tag path moved out of `qc/analyzers` to their own area**, `icc26/site1/env_monitoring/particle-counter-01/sample-analyzed` — a particle counter watches a room, it does not run analyses on samples. Four runtime files, no ACL change and no database change (`device_id` is the last path segment, so `em.reading` never noticed), and the counter dropped out of `lims-bridge`'s `qc/analyzers/+/sample-analyzed` wildcard, where it had been arriving and being discarded for having no `sample_id`. [`06-poll-particle-counter.md`](06-poll-particle-counter.md), [`../talk-tracks/06-poll.md`](../talk-tracks/06-poll.md) |
| **07 — the composite** | **Built and broker-verified 2026-08-30, the day its spec was written.** One script module `sample_chain` and one Event Stream `07_chain/lims-review` whose source is MQTT Engine's own MQTT source on `icc26/site1/qc/lims/sample-results-released` — **no new table, no ACL change, no new datasource, and nothing in patterns 1-6 touched**. On a review it lands one composite on `icc26/site1/qc/sample-chain`, joining what pattern 5 wrote to `bes.batch_event` and pattern 6 wrote to `em.reading` onto the sample the LIMS just released. **It computes nothing** — `qualified_window` and `status` were both decided at ingest by the pattern that produced them. **All eight checkpoints closed** on two real approvals (`S-20260830-0085` pass, `S-20260830-0084` fail) and two transient probes: `GROWTH` + `qualified_window: true` for a sample drawn now, `IDLE` + `false` + nothing empty for one drawn between batches, `age_s` 2.5 s live, and a null lookup published with its reason beside it. Applied with `scan` — no restart, no Designer. **Talk track written.** **Revised 2026-09-06: it no longer publishes a composite per review.** 07 now speaks only when the sample violated something and names what in `values.violations` — an environmental excursion (`em.reading.status`, the particle counter's Dirty button) or a failed analyst review. Clean samples publish nothing, and the topic moved to `icc26/site1/qc/deviation`. The gate still computes nothing: it reads the two flags their owning modules already set, and `qualified_window` is reported but deliberately not a trigger, being false for four operations of five. [`07-sample-chain.md`](07-sample-chain.md) § *As built*, [`../talk-tracks/07-sample-chain.md`](../talk-tracks/07-sample-chain.md) |
...
~~**Talk tracks for 03, 05 and 06**, and 04's rewrite~~ — **all four closed 2026-09-06.**
[`../talk-tracks/`](../talk-tracks/) now holds seven, one per pattern, per the master plan's
two-document convention. 04's was rewritten rather than patched: it described a queue the analyzer
filled, which is not what the code had done since 2026-08-26. Three corrections came out of
writing them, and all three are in the talk tracks rather than only here — pattern 3 publishes no
`meta.mechanism` at all (so the firehose shows **four** mechanisms, not seven), pattern 5's MVP
shapes survived 07 untouched, and pattern 6's `config/poll_interval_s` is decorative, so the stall
demo uses `config/enabled`.
...
side* for the particle counter, so `em.reading` is read with
...
stopped particle counter shows a growing `age_s` rather than a silence — and the document's `seq` carries
...
**Pattern 6 built** (master plan Order item 4). A new `sim-particle-counter` service, the `em` schema, the
`particle_counter` UDT, `particle_counter_poll`, and Event Stream `06_poll/particle-counter-result` —
...
an Ignition tag write. Three services changed: the LIMS opens its entry from pattern 1, the analyzer
...
- **The Countess is out of the demo.** Its server, compose service, `opc-connection/cell_counter`
  and the `cell_counter` UDT type stay in the repo as the worked example; the `countess-01`
  instance is deleted and its MQTT publish will not be wired.
- ~~**Pattern 6 moved into `qc/analyzers`.**~~ **Superseded 2026-09-13** — moved again, to its own `env_monitoring` area. [`06-poll-particle-counter.md`](06-poll-particle-counter.md) § *Log*.
