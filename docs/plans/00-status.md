...
| **03 — OPC UA → MQTT** | Analyzer path built and broker-verified 2026-08-20 (`S-00140` watched live). Device id is `cell-analyzer-01`. The Countess is out of the demo. **2026-08-26: the instrument gained its own sample-login screen on :8087 and no longer free-runs** — `SAMPLE_INTERVAL_S` defaults to 0, and the sample id is typed in by a person. [`03-opcua-analyzer-playbook.md`](03-opcua-analyzer-playbook.md) § 8b. |
