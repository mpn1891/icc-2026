...
> **Updated 2026-08-29.** Conference is ~4 weeks out.
...
`WHERE device_id = ? AND occurred_at <= ? ORDER BY occurred_at DESC, id DESC LIMIT 1` — so 07
has one query idiom for both of its flags rather than two. **`em.reading` is not in the
