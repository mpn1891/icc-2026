...
The write is the stream's transform (`qc.analyzer_result`); the publish is its MQTT handler (`icc26/site1/qc/analyzers/cell-analyzer-01/sample-analyzed`, QoS 1). Store first, publish second, and the store is wrapped so it cannot throw: the handler's `failureStrategy` is ABORT, so a Postgres outage must cost the row and never the publish.
