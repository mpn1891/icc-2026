...
-- Debezium can create its own publication, but doing it explicitly means the set
-- of captured tables is a reviewable, diffable artifact in this repo rather than
-- an implicit side effect of a connector's config.
...
-- the primary key, and the demo can't show what actually changed. It costs WAL
-- volume, which is the honest tradeoff to mention: fine for these tables, not
