...
The Postgres schema (`compose/postgres/initdb/02-schema.sql`) now has `status` / `verified_at`
on `lims.sample_result` and the `lims.webhook_delivery` outbox. `mes.batch_event` in the same
file has **no consumer at all**, since Odoo replaces the hand-made MES table.
